...
```{r packages}
pacman::p_load(tidyverse, MuMIn, pastecs, WRS2, purr, data.table, magrittr, dplyr)
```
...
new_df <- new_df[-c(13, 34, 60, 71, 83, 98, 104, #participants with numbers as ID
                    119, 120, 126, 130), ] # participants with names as ID
# Participants med navne, der skal fjernes: 
# "Banan", "bananasplit med softice", "fluy", "Julemor", 
...
# De andre har glemt at udfylde det hele
...
```{r første udregninger}

mean(new_df$hastighed1)
...
by_cond <- group_by(new_df, new_df$condition)
#group_by(new_df, condition)
mean(new_df$hastighed1)
