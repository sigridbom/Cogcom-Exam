...
```{r visualizing the data in general - make better}
...
```{r Hyp1a - t-test assumptions hastighed/condition}
...
```{r Hyp1a visual inspection of the transformed data}
...
```{r Hyp1a - t-test on hastighed1 and condition}
...
## new numbers change them
From the parametric t-test on the sqrt transformed data: do not use!
Using an independent t-test, we found that condition B ("smadrede") did not significantly increase the average speed estimate of the car, t(x) = x, p x 5, (M condA = x, M condB = x, SD condA = x, SD condB = x)
...
```{r Hyp1a - visualizing the t-tests}
...
# trying the non-transformed data THIS ONE IS USED
new_df$condition <- as.factor(new_df$condition)
...
```{r Hyp1b - glas spørgsmål - logistic regression}
...
round(boot::inv.logit(0.570545), digits = 4)
# = 63,89%


# the probability of answering yes in condition b
round(boot::inv.logit(0.570545 - 0.004819), digits = 4)
# = 63,78%
...
```{r parti - køretimer vs computertimer + kørekort}
...
```{r Hyp 2 ændre svar logistic regression (multiple)}
...
```
A significant effect of the younger group was found, beta = -1.4018, SE = 0.28, z-value = -5.02, p<.001 (p=5.09e-07), while the other age group only came close to being significant, beta = -1.0492, SE = 0.54, z-value = -1.93, p>.05 (p = 0.0534).


```{r Hyp 2 det nye svar - forskel i procent}
...
# visualizing the data ## work in progress

ggplot(df_svar_gender_ung, aes(køn, speed_change_plus, fill = køn)) +
...
#ggplot(df_svar_group_ung, aes(køn, speed_change_plus, fill = køn)) + geom_quantile()
...
*There was found a significant difference between the mean change of speed estimate for men and women in the age group below 20 years of age, t(10) = 2.25, p<.05 (p = 0.04824) (M women = 14.87, M men = 31.23, SD women = 10.53, SD men = 17.41)*
