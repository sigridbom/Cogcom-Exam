...
```{r Hyp 1b - glas spørgsmål - logistic regression} 

# see class 10 notes filled
new_df$glasskår <- as.factor(new_df$glasskår)

#change levels
new_df$glasskår <- relevel(new_df$glasskår, "n")
#check levels (n og condA skal være baseline)
...
ggplot(new_df, aes(x = glasskår, fill = condition)) +
...
```{r confusion matrix - glasskår}

library(caret)

#let's remind ourselves the model: yes glasskår (jagged is level 1) ~ condition
summary(m) 
...
actual_categories = new_df$glasskår
...
#make a new column to see more easily what our model predicted: if probability is less than 0.5 - it predicted 'nej', otherwise - it predicted yes
pred_df$predicted_category = ifelse(pred_df$predicted_probs < 0.5, "n", "j")
...
confusionMatrix(pred_df$predicted_category, pred_df$actual_categories, positive ="j")
...
ggplot(new_df, aes(glasskår, condition, color = condition, shape = condition)) + geom_point() 

#spread points out - so much better
ggplot(new_df, aes(glasskår, condition, color = condition, shape = condition)) + geom_jitter(width = .4, height = .4) 
```



```{r participants - køretimer vs computertimer + kørekort}
...
summary(df_old$køn == "Kvinde")
summary(df_ung$køn == "Kvinde")
...
# = 93.65% af dem over 20 år har kørekort
...
# = 18.52% af dem under 20 år har kørekort
...
```{r Hyp 2a ændre svar percentage}
...
summary(df_svar$køn == "Kvinde" & df_svar$under_20 == TRUE)
summary(df_svar$køn == "Mand" & df_svar$under_20 == TRUE)

summary(df_svar$køn == "Kvinde" & df_svar$under_20 == FALSE)
summary(df_svar$køn == "Mand" & df_svar$under_20 == FALSE)
...
```{r Hyp 2a ændre svar - simple logistic regression}

new_df$ændre_svar <- as.factor(new_df$ændre_svar)
levels(new_df$ændre_svar)
new_df$ændre_svar <- relevel(new_df$ændre_svar, "n")
...
#m_svar <- glm(ændre_svar ~ under_20, family = binomial, data = no_outliers)
...
I.e. choosing to change one's answer can not be concluded to significantly be modulted by age (below or above 20 years). men det er lige på vippen!
...
#changing classes
new_df$alder <- as.numeric(new_df$alder)
new_df$køn <- as.factor(new_df$køn)
...
summary(new_df$ændre_svar == "j")
# 21 observations

#making a new dataframe with only these observations
df_svar <- subset(new_df, new_df$ændre_svar == "j")
...
# doing the same but with km/h instead of percent
df_svar$kmh_diff <- df_svar$hastighed1 - df_svar$hastighed2
df_svar$kmh_diff <- sqrt((df_svar$kmh_diff)^2)

#calculating the mean of this
round(mean(df_svar$kmh_diff), digits = 2)
round(sd(df_svar$kmh_diff), digits = 2)
...
df_svar_gender <- df_svar %>% group_by(df_svar$køn)
...
WRS2::yuen(speed_change_plus ~ køn, data = df_svar_gender)
...
WRS2::yuen(speed_change_plus ~ køn, data = df_svar_gender_ung)
...
WRS2::yuen(speed_change_plus ~ køn, data = df_svar_gender_old)

# visualizing the data - this is the used graph, do not change!
ggplot(df_svar_gender_ung, aes(x = køn, y = speed_change_plus, fill = køn)) +
...
# plot hvor det går fra estimate 1 til estimate 2 i gennemsnit ift køn og kun den unge gruppe
ggplot(df_svar_gender_ung, aes(x = c(mean_hastighed1, mean_hastighed2), speed_change_plus, fill = køn)) + geom_point(stat = 'summary', fun.y = mean) +
...
#first, remove outliers from the whole data (everything 3 sd away from the mean)
#calculate z score
df_svar$rt_z = (df_svar$speed_change_plus - mean(df_svar$speed_change_plus)/sd(df_svar$speed_change_plus)

#filter data 
no_outliers_svar = df_svar %>% filter(rt_z > -3 & rt_z < 3)

#subsetting data
girls <- no_outliers_svar %>% filter(køn == "Kvinde" & under_20 == 'TRUE')
boys <- no_outliers_svar %>% filter(køn == "Mand" & under_20 == 'TRUE')

############ Checking Assumptions for girls
#histogram
ggplot(girls, aes(x = speed_change_plus)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.4) +
  ggtitle("Distribution of percentage change in speed estimate1 for girls") +
  stat_function(fun = dnorm, args = list(mean = mean(girls$speed_change_plus, na.rm = TRUE), sd = sd(girls$speed_change_plus, na.rm = TRUE)), colour = "red", size = 1) +
  theme_minimal()

#qqplot
ggplot(girls, aes(sample = speed_change_plus)) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
  ggtitle("Qqplot for percentage change in speed estimate for young girls") +
  theme_minimal()

############ Checking Assumptions for boys
#histogram
ggplot(boys, aes(x = speed_change_plus)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.4) +
  ggtitle("Distribution of percentage change in speed estimate1 for boys") +
  stat_function(fun = dnorm, args = list(mean = mean(boys$speed_change_plus, na.rm = TRUE), sd = sd(boys$speed_change_plus, na.rm = TRUE)), colour = "red", size = 1) +
  theme_minimal()

#qqplot
ggplot(boys, aes(sample = speed_change_plus)) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
  ggtitle("Qqplot for percentage change in speed estimate for young boys") +
  theme_minimal()

numbers1 <- round(pastecs::stat.desc(cbind(girls$speed_change_plus, boys$speed_change_plus), basic = FALSE, norm = TRUE), digits = 2)

# try transforming a doing a normal t.test


#logging the data for both conditions
girls <- girls %>% mutate(rt_log = log(girls$speed_change_plus), 
                                    rt_sqrt = sqrt(girls$speed_change_plus),
                                    rt_inv = 1/girls$speed_change_plus)

boys <- boys %>% mutate(rt_log = log(boys$speed_change_plus), 
                                    rt_sqrt = sqrt(boys$speed_change_plus),
                                    rt_inv = 1/boys$speed_change_plus)

#checking numeric values for both conditions with the logged data
numbers2 <- round(pastecs::stat.desc(cbind(girls$rt_log, girls$rt_sqrt, girls$rt_inv, boys$rt_log, boys$rt_sqrt, boys$rt_inv), basic = F, norm = TRUE), digits = 2)


#Check assumptions again
############ Checking Assumptions for Condition A (best candidates: log and sqrt)
#histogram
ggplot(girls, aes(x = rt_log)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("Distribution of percentage change in speed estimate data (log transformed) for girls") +
  stat_function(fun = dnorm, args = list(mean = mean(girls$rt_log, na.rm = TRUE), sd = sd(girls$rt_log, na.rm = TRUE)), colour = "darkgreen", size = 1) +
  theme_minimal()

#qqplot
ggplot(girls, aes(sample = rt_log )) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
  ggtitle("Qqplot for percentage change in speed estimate data (log transformed) for girls") +
  theme_minimal()

#histogram
ggplot(girls, aes(x = rt_sqrt)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("Distribution of percentage change in speed estimate data (square root transformed) for girls") +
  stat_function(fun = dnorm, args = list(mean = mean(girls$rt_sqrt, na.rm = TRUE), sd = sd(girls$rt_sqrt, na.rm = TRUE)), colour = "darkgreen", size = 1) +
  theme_minimal()
...
ggplot(condA, aes(sample = rt_sqrt)) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
  ggtitle("Qqplot for percentage change in speed estimate data (squareroot transformed) for girls") +
  theme_minimal()

#### assumptions for boys

ggplot(boys, aes(x = rt_log)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("Distribution of percentage change in speed estimate data (log transformed) for boys") +
  stat_function(fun = dnorm, args = list(mean = mean(boys$rt_log, na.rm = TRUE), sd = sd(boys$rt_log, na.rm = TRUE)), colour = "darkgreen", size = 1) +
  theme_minimal()

#qqplot
ggplot(boys, aes(sample = rt_log )) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
  ggtitle("Qqplot for percentage change in speed estimate data (log transformed) for boys") +
  theme_minimal()

#histogram
ggplot(boys, aes(x = rt_sqrt)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("Distribution of percentage change in speed estimate data (square root transformed) for boys") +
  stat_function(fun = dnorm, args = list(mean = mean(boys$rt_sqrt, na.rm = TRUE), sd = sd(boys$rt_sqrt, na.rm = TRUE)), colour = "darkgreen", size = 1) +
  theme_minimal()

#qqplot
ggplot(boys, aes(sample = rt_sqrt)) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
  ggtitle("Qqplot for percentage change in speed estimate data (squareroot transformed) for boys") +
  theme_minimal()

numbers1
numbers2

#I'll continue with log transformed data

transformed_data_new <- bind_rows(girls, boys) 
view(transformed_data_new)

```


```{r Hyp 2b t-test on transformed data}

#### parametric tests on the transformed data
t.test(transformed_data_new$rt_log ~ transformed_data_new$køn, transformed_data_new)

#calculating the means
girls %>% summarise(mean(speed_change_plus))
boys %>% summarise(mean(speed_change_plus))

# we need the mean and the SD of the data when reporting the results
# calculating the SD of the mean of estimating spee 1 data according to condition on the transformed data
girls %>% summarise(sd(speed_change_plus))
boys %>% summarise(sd(speed_change_plus))
...
# visualizing the data - this is the used graph, do not change! TRANSFORMED
ggplot(transformed_data_new, aes(x = køn, y = rt_log, fill = køn)) +
...
new_df$køretimer <- as.numeric(new_df$køretimer)

# removing outliers 
new_df$ct_z = (new_df$computertimer - mean(new_df$computertimer))/sd(new_df$computertimer) 
new_df$kt_z = (new_df$køretimer - mean(new_df$køretimer))/sd(new_df$køretimer)
...
############ Checking Assumptions for df_ung_no_kt (young group køretimer)
#histogram
ggplot(df_ung_no_kt, aes(x = df_ung_no_kt$køretimer)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.4) +
  ggtitle("Distribution of driving hours young group") +
  stat_function(fun = dnorm, args = list(mean = mean(df_ung_no_kt$køretimer, na.rm = TRUE), sd = sd(df_ung_no_kt$køretimer, na.rm = TRUE)), colour = "blue", size = 1) +
  theme_minimal()

#qqplot
ggplot( df_ung_no_kt, aes(sample = køretimer)) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
...
############ Checking Assumptions for df_old_no_kt (old group køretimer)
#histogram
ggplot(df_old_no_kt, aes(x = df_old_no_kt$køretimer)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.4) +
  ggtitle("Distribution of driving hours old group") +
  stat_function(fun = dnorm, args = list(mean = mean(df_old_no_kt$køretimer, na.rm = TRUE), sd = sd(df_old_no_kt$køretimer, na.rm = TRUE)), colour = "blue", size = 1) +
  theme_minimal()

#qqplot
ggplot( df_old_no_kt, aes(sample = køretimer)) +
  stat_qq() +
  stat_qq_line(colour = 'red') +
...
#stat.desc
round(pastecs::stat.desc(cbind(df_ung_no_ct$computertimer, df_old_no_ct$computertimer, df_ung_no_kt$køretimer, df_old_no_kt$køretimer), basic = FALSE, norm = TRUE), digits = 2)
...
df_ung_no_kt <- df_ung_no_kt %>% mutate(ung_kt_log = log(df_ung_no_kt$køretimer), 
                                    ung_kt_sqrt = sqrt(df_ung_no_kt$køretimer),
                                    ung_kt_inv = 1/df_ung_no_kt$køretimer)
...
df_old_no_kt <- df_old_no_kt %>% mutate(old_kt_log = log(df_old_no_kt$køretimer), 
                                    old_kt_sqrt = sqrt(df_old_no_kt$køretimer),
                                    old_kt_inv = 1/df_old_no_kt$køretimer)
...
#non-parametric t-test (robust t-test)
WRS2::yuen(køretimer ~ under_20, data = new_df)
sd(df_ung$køretimer)
sd(df_old$køretimer)
...
```{r parti - køretimer + computertimer visualization}
...
ggplot(new_df, aes(under_20, køretimer, fill = under_20)) +
  theme_minimal() +
