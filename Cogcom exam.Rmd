...
new_df <- new_df[-c(13, 34, 60, 71, 83, 98, 104, 114, #participants with numbers as ID
...
drivers_lice <- data.frame(ID = sample(letters, 147, rep = TRUE))
drivers_lice %>% 
  group_by(new_df$kørekort) %>% 
  summarise(no_rows = length(ID))

round(70/147, digits = 4)
# = 47.62% har kørekort generelt

drivers_lice_old <- data.frame(ID = sample(letters, 64, rep = TRUE))
drivers_lice_old %>% 
  group_by(df_old$kørekort) %>% 
  summarise(no_rows = length(ID))

round(60/64, digits = 4)
# = 93.75% af dem over 20 år har kørekort

drivers_lice_ung <- data.frame(ID = sample(letters, 83, rep = TRUE))
drivers_lice_ung %>% 
  group_by(df_ung$kørekort) %>% 
  summarise(no_rows = length(ID))

round(15/83, digits = 4)
# = 18.07% af dem under 20 år har kørekort
...
nyt_svar <- data.frame(ID = sample(letters, 147, rep = TRUE))
nyt_svar %>% 
  group_by(new_df$ændre_svar) %>% 
  summarise(no_rows = length(ID))

round(23/147, digits = 4)
# = 15.65% changed their answer, rated a second time

# calculating the number of YOUNG people who changed their answers
class(df_ung$ændre_svar)

nyt_svar_ung <- data.frame(ID = sample(letters, 83, rep = TRUE))
nyt_svar_ung %>% 
  group_by(df_ung$ændre_svar) %>% 
  summarise(no_rows = length(ID))

round(17/83, digits = 4)
# = 20.48% 

# calculating the number of OLD people who changed their answers
class(df_old$ændre_svar)

nyt_svar_old <- data.frame(ID = sample(letters, 64, rep = TRUE))
nyt_svar_old %>% 
...
boot::inv.logit(-1.3564)
# = 20.48% significant chance of changing one's answer

boot::inv.logit(-1.3564 - 0.9122)
# = 9.38%, significant at p<0.1??

round(boot::inv.logit(-2.2687), digits = 4)
...
```{r ændre svar - coherent eller ej}

df_nytsvar <- filter(new_df, new_df$ID, new_df$alder, new_df$køn, new_df$condition, new_df$estimate, new_df$ændre_svar, new_df$hastighed1, new_df$hastighed2)
...
class(new_df$ændre_svar)
class(new_df$hastighed1)
class(new_df$hastighed2)
...
```{r t-test between computer hours and driving hours}

ggplot(new_df, aes(under_20, computertimer, fill = under_20)) +
  theme_minimal() +
  labs(x = "Age below 20 (TRUE) or equals to and above 20 years (FALSE)", y = "Average amount of hours spent playing computer etc weekly") +
  geom_boxplot(width = 0.5) +
  ggtitle("Average amount of hours spent playing computer etc weekly depending on age") +
  stat_summary(fun.data = mean_se, geom = "errorbar", color = 'black', width = 0.1) + coord_cartesian(ylim = c(0,32))

ggplot(new_df, aes(under_20, køretimer, fill = under_20)) +
  theme_minimal() +
  labs(x = "Age below 20 (TRUE) or equalt to or above 20 years (FALSE)", y = "Average amount of hours spent driving weekly per participant") +
  geom_boxplot(width = 0.5) +
  ggtitle("Average hours spent driving weekly depending on age") +
  stat_summary(fun.data = mean_se, geom = "errorbar", color = 'black', width = 0.1) + coord_cartesian(ylim = c(0,17))
...
#stat.desc
round(pastecs::stat.desc(cbind(df_ung$computertimer, df_old$computertimer), basic = FALSE, norm = TRUE), digits = 2)

#non-parametric t-test (robust t-test)
WRS2::yuen(computertimer ~ under_20, data = new_df)
sd(new_df$computertimer)
...
```
Performing a robust t-test it was found that there is a significant difference between the average hours spent playing computer or other gaming consoles weekly between people younger than 20 years of age and people equals to or older than 20 years of age, t(77.74) = 3.50, p<.001 (M_young = 4.19, SD = 7.38, M_old = 2.38, SD = 5.48). 

Likewise when performing a robust t-test on the difference between the agerage number of hours spent driving a vehicle weekly there was found to be a significant difference between the two groups of younger than 20 years of age and older than or equals to 20 years of age, t(43.32) = 4.51, p<0.001 (M_young = 1.42, SD = 5.31, M_old = 3.82, SD = 3.88)
